...
def get_freq(word, dfs):
  return np.sum(pd.concat([df.loc[:, word] for df in dfs if word in df.columns]))
...
if __name__ == "__main__":
  ...
  s1_freq = tmps[0].drop('tags', 1).div(s1_total_wc, 0)
  s2_freq = tmps[1].drop('tags', 1).div(s2_total_wc, 0)
  s3_freq = tmps[2].drop('tags', 1).div(s3_total_wc, 0)
  s4_freq = tmps[3].drop('tags', 1).div(s4_total_wc, 0)

  tmps_freq = [s1_freq, s2_freq, s3_freq, s4_freq]

  word_freq = [(word, get_freq(word, tmps_freq)) for word in feature_list]
  ...
  df.to_csv(join(OUTPUT_DIR, "doc_feature_top_word_frequencies_100_to_2100.csv"))
