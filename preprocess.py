...
def tfidf(word, dfs, transformer, doc_word_sums):
  """
  Given a word, compute the sum of tf-idf values across all observations in the four genres
  """
  df_word_sum_info = [(df.loc[:,word], doc_word_sum) for df, doc_word_sum in zip(dfs, doc_word_sums) if word in df.columns]
  ...
  # convert icidf values to ifidf values based on the formula in 
...
def docs(toplevel_file, limit=8000):
...
if __name__ == "__main__":
  ...
  word_and_tfidf = [(word, tfidf(word, tmps, transformer, doc_word_sums)) for word in feature_list]
  ...
  df.to_csv(join(OUTPUT_DIR, "doc_feature_matrix.csv"))
